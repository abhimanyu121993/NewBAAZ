                        ...
                        <div class="card-content">
                            <div class="card-body card-dashboard">
                                <div class="table-responsive">
                                    <table class="display nowrap" id="order_pending" style="width:100% !important;">
                                            ...
                                            @if ($pendingorders)
                                                @foreach ($pendingorders as $order)
                                                    <tr>
                                                        ...
                                                        @canany(['Pending_orders_edit', 'Pending_orders_delete'])
                                                            <form id="allotworkshop" action="{{ route('Backend.allotWorkshop') }}"
                                                                method="POST">
                                                                @csrf
                                                                <td>
                                                                    <input type="hidden" name="oid"
                                                                        value="{{ $order->id ?? '' }}" />
                                                                    <select style="width:200px;" class="form-select" name='wid'
                                                                        required>
                                                                        <option selected disabled value="">--Select Workshop--
                                                                        </option>
                                                                        @foreach ($workshops as $shop)
                                                                            <option
                                                                                {{ !isset($order) ? '' : ($order->assigned_workshop == $shop->id ? 'selected' : '') }}
                                                                ...
                                                                <td>
                                                                    <button type="submit"
                                                                        class="btn-icon btn btn-primary btn-round btn-sm"><i
