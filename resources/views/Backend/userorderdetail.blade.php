                    ...
                    @if ($orderDetails)
                        @foreach ($orderDetails->order_details as $orderd)
                            <tr>
                                ...
                                <td>{{ $orderd->modelmapservice->discounted_price ?? '' }}
                                </td>
                            </tr>
                        @endforeach
                        @role('Superadmin|Workshop')
                            <tr>
                                ...
                                <td colspan="2" ><a href="{{ route('Backend.jobcard.edit', $order->id) }}" class="btn btn-primary">Job
                                        Card</a></td>
                            </tr>
                        @endrole
                    @else
                        <tr>
                            <td colspan="3" class="text-center">Service not available for this order</td>
                        </tr>
                    @endif
                </tbody>
            </table>
